...
class NamingConventionPanel(ttk.Frame):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._custom_tags: list[str] = list(DEFAULT_TAG_LIBRARY[:MAX_CUSTOM_TAGS])
    ...
    def _rebuild_tag_buttons(self) -> None:
        for w in self._tag_btn_frame.winfo_children():
            w.destroy()
        for tag in self._custom_tags[:MAX_CUSTOM_TAGS]:
            ttk.Button(
    ...
    def _add_current_to_library(self) -> None:
        ...
        if text in self._custom_tags:
            ...
            return
        if len(self._custom_tags) >= MAX_CUSTOM_TAGS:
            messagebox.showwarning("提示", f"常用标签已满（最多 {MAX_CUSTOM_TAGS} 个），请先删除一个")
            return
        self._custom_tags.insert(0, text)
        self._custom_tags = self._custom_tags[:MAX_CUSTOM_TAGS]
    ...
    def _apply_config(self, cfg: dict[str, Any]) -> None:
        ...
        if isinstance(lib, list) and lib:
            self._custom_tags = [str(t) for t in lib[:MAX_CUSTOM_TAGS]]
        ...
        for i, fname in enumerate(files):
            ...
            try:
                if legacy:
                    ...
                    new_name, date_ok = build_filename(fields, idx)
                    if not date_ok:
                        note = "日期异常"
    ...
    def _execute_rename(self) -> None:
        ...
        self._custom_tags = add_tags_to_library(self._custom_tags, tags, MAX_CUSTOM_TAGS)
        self._rebuild_tag_buttons()
        self._schedule_save()
        self._refresh_preview()
