...
class OverlayModule(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ctrl = ttk.LabelFrame(self, text="图层列表", padding=6)
        ...
        ttk.Button(ctrl, text="加载底图", command=self.load_base).grid(row=1, column=0, columnspan=2, sticky="ew", pady=2)
        self.base_info = StringVar(value="未加载")
        ttk.Label(ctrl, textvariable=self.base_info, wraplength=220, font=("", 8)).grid(
            row=2, column=0, columnspan=2, sticky="w", pady=2)

        ttk.Separator(ctrl, orient=HORIZONTAL).grid(row=3, column=0, columnspan=2, sticky="ew", pady=6)
        ...
        ttk.Label(ctrl, textvariable=self.video_folder_info, wraplength=220, font=("", 8)).grid(
            row=6, column=0, columnspan=2, sticky="w")

        nav = ttk.Frame(ctrl)
        nav.grid(row=7, column=0, columnspan=2, sticky="ew", pady=4)
        ...
        ttk.Label(ctrl, textvariable=self.video_preview_info, wraplength=220, font=("", 8)).grid(
            row=8, column=0, columnspan=2, sticky="w")
        ttk.Label(ctrl, text="视频缩放%:").grid(row=9, column=0, sticky="w")
        vs = ttk.Scale(ctrl, from_=10, to=200, variable=self.video_scale_pct, orient=HORIZONTAL,
                        command=self._on_video_scale)
        vs.grid(row=9, column=1, sticky="ew", pady=2)

        ttk.Separator(ctrl, orient=HORIZONTAL).grid(row=10, column=0, columnspan=2, sticky="ew", pady=6)

        # --- Logo层 ---
        ttk.Checkbutton(ctrl, text="图层3：Logo", variable=self.logo_enabled,
                        command=self._on_layer_toggle).grid(row=11, column=0, columnspan=2, sticky="w")
        ttk.Button(ctrl, text="加载Logo", command=self.load_logo).grid(row=12, column=0, columnspan=2, sticky="ew", pady=2)
        self.logo_info = StringVar(value="未加载")
        ttk.Label(ctrl, textvariable=self.logo_info, wraplength=220, font=("", 8)).grid(
            row=13, column=0, columnspan=2, sticky="w", pady=2)
        ttk.Label(ctrl, text="Logo缩放%:").grid(row=14, column=0, sticky="w")
        logo_scale_row = ttk.Frame(ctrl)
        logo_scale_row.grid(row=14, column=1, sticky="ew", pady=2)
        ...
        ttk.Separator(ctrl, orient=HORIZONTAL).grid(row=15, column=0, columnspan=2, sticky="ew", pady=6)

        # --- 时间控制 ---
        self.time_frame = ttk.Frame(ctrl)
        self.time_frame.grid(row=16, column=0, columnspan=2, sticky="ew")
        ...
        ttk.Label(ctrl, text="选中图层坐标(原分辨率):").grid(row=17, column=0, columnspan=2, sticky="w", pady=(8, 2))
        self.coord_x = StringVar(value="0")
        self.coord_y = StringVar(value="0")
        self.coord_w = StringVar(value="0")
        self.coord_h = StringVar(value="0")
        cf = ttk.Frame(ctrl)
        cf.grid(row=18, column=0, columnspan=2, sticky="w")
        ...
        self.batch_btn = ttk.Button(ctrl, text="开始批量处理", command=self.batch_process)
        self.batch_btn.grid(row=20, column=0, columnspan=2, sticky="ew", pady=2)
        ...
        ttk.Label(preview_f, text="点击选中图层 | 拖动定位 | 滚轮缩放 | 边缘吸附 | R重置",
    ...
    def _update_preview_label(self):
        ...
        self.video_preview_info.set(f"当前预览：第 {n} / {len(self.video_files)} 个 | {name}")
    ...
    def _apply_smart_layouts(self):
        if self.base_w <= 0 or not self.bg_path:
            return
    ...
    def _update_batch_btn(self):
        combo = detect_combo(self.bg_enabled.get(), self.video_enabled.get(), self.logo_enabled.get())
    ...
    def _on_logo_scale(self, *_args):
        if self._selected == "logo" or self.logo_path:
            self._apply_logo_scale()
            self._redraw()
            self._notify()

    def _apply_logo_scale(self):
        if not self.bg_path or not self.logo_path:
            return
        rx, ry, rw, rh = calculate_logo_default(
            self.base_w, self.base_h, self.logo_orig_w, self.logo_orig_h, self.logo_scale_pct.get())
        cx, cy, cw, ch = real_to_canvas(rx, ry, rw, rh, self.base_w, self.base_h)
        self._layer_rects["logo"] = [cx, cy, cx + cw, cy + ch]
    ...
    def reset_selected_layer(self):
        if self._selected == "video" and self.bg_path:
            rx, ry, rw, rh = calculate_smart_layout(
                self.base_w, self.base_h, self.video_orig_w, self.video_orig_h)
            ...
            cx, cy, cw, ch = real_to_canvas(rx, ry, rw, rh, self.base_w, self.base_h)
            self._layer_rects["video"] = [cx, cy, cx + cw, cy + ch]
        elif self._selected == "logo" and self.bg_path:
            self._apply_logo_scale()
    ...
    def load_state(self, state: dict):
        ...
        self.logo_enabled.set(logo.get("enabled", False))
        ...
        if pos.get("w") and self.base_w:
            cx, cy, cw, ch = real_to_canvas(pos["x"], pos["y"], pos["w"], pos["h"], self.base_w, self.base_h)
            self._layer_rects["video"] = [cx, cy, cx + cw, cy + ch]
        lpos = logo.get("position", {})
        if lpos.get("w") and self.base_w:
            cx, cy, cw, ch = real_to_canvas(lpos["x"], lpos["y"], lpos["w"], lpos["h"], self.base_w, self.base_h)
            self._layer_rects["logo"] = [cx, cy, cx + cw, cy + ch]
            ...
            for i, (vf, outp) in enumerate(tasks, 1):
                try:
                    ...
                    cmd = build_combo_cmd(
                        ...
                        self.bg_path, vf, self.logo_path, outp, vpos, lpos, dur,
                    )
                    r = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                       creationflags=_subprocess_flags())
                    if r.returncode == 0:
                        ok += 1
                        self.log_fn(f"叠加 [{i}/{len(tasks)}] OK {outp.name}")
                    else:
                        fail += 1
                        err = format_ffmpeg_stderr(r.stderr)
                        last_err = err
                        filt = next((cmd[j + 1] for j, a in enumerate(cmd) if a == "-filter_complex"), "")
                        self.log_fn(f"叠加 [{i}/{len(tasks)}] 失败 {outp.name}")
                        self.log_fn(f"  时长: {dur}s | filter: {filt}")
                        for line in err.splitlines():
                            self.log_fn(f"  FFmpeg: {line}")
                except Exception as e:
                    fail += 1
                    last_err = str(e)
                    self.log_fn(f"叠加失败 [{i}/{len(tasks)}]: {e}")
            self._processing = False
...
class OverlayEditorWindow(Toplevel):
    ...
    def __init__(self, parent, ffmpeg="ffmpeg", ffprobe="ffprobe",
        ...
        top.pack(fill=X)
        ttk.Label(top, text=f"输出文件夹: {output_dir or '(未设置)'}", font=("", 8)).pack(side=LEFT, padx=4)
        ttk.Label(top, text="批量处理素材文件夹内的视频（预览仅用于定位）",
