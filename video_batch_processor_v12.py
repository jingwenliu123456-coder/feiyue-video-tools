...
class VideoBatchProcessor:
    def __init__(self, root):
        self.root = root
        self.root.title("视频批量处理工具 v12.0")
    ...
    def setup_ui(self):
        ...
        ttk.Label(scrollable_frame, text="视频批量处理工具 v12.0", font=("Microsoft YaHei", 18, "bold")).pack(pady=(0,5))
        ttk.Button(scrollable_frame, text="使用说明", command=self.show_help).pack(anchor=tk.W, padx=10, pady=5)

        # 左右两栏重命名（复刻v8）
        rename_frame = ttk.LabelFrame(scrollable_frame, text="批量重命名（源→目标）", padding=10)
        ...
        self.src_listbox = tk.Listbox(left_frame, height=8, width=55, exportselection=False)
        ...
        ttk.Label(mid_frame, text="→", font=("Arial", 20)).pack(pady=(40,5))
        ...
        self.dst_listbox = tk.Listbox(right_frame, height=8, width=55, exportselection=False)
        ...
        ttk.Button(btn_frame, text="一键批量处理", command=self.start_process, style="Accent.TButton").pack(side=tk.LEFT, padx=10)
    ...
    def show_help(self):
        messagebox.showinfo("使用说明",
            "1. 选择源文件夹（左）和目标文件夹（右）\n"
            "2. 左列选中文件 → 点击复制 → 剪贴板显示\n"
            "3. 右列选中对应行 → 点击粘贴到选中 → 完成重命名预设\n"
            "4. 勾选需要的功能，设置参数\n"
            "5. 点击一键批量处理\n\n"
            "水印持续秒数填0=全程显示，填10=只显示前10秒后自动消失\n"
    ...
    def process_all(self):
        ...
        if len(src_files) != len(dst_files):
            messagebox.showerror("错误", "源列表和目标列表数量不一致，请刷新两列")
            return
        if not src_files:
            self.log("没有文件需要处理")
            return
        ...
        for idx, (src_name, dst_name) in enumerate(zip(src_files, dst_files), 1):
