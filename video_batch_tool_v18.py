...
"""
视频批处理工具 V18
全局输入/输出与重命名模块严格分离 | 贴图Logo | 音频拼接 | 撤销备份
"""
...
from core.overlay_engine import build_combo_cmd, probe_duration, list_videos_in_folder

FFMPEG_PATH = "ffmpeg"
FFPROBE_PATH = "ffprobe"
CONFIG_FILE = "video_batch_config_v18.json"
...
class VideoBatchTool:
    ...
    def check_ffmpeg(self):
        ok, out = run_ffmpeg([FFMPEG_PATH, '-version'])
        if ok:
            self.log(f"FFmpeg 已就绪: {out.splitlines()[0] if out else 'OK'}")
        else:
            self.log("FFmpeg 未找到，请检查环境变量或 FFMPEG_PATH")
    ...
    def build_ui(self):
        ...
        row += 1
        self.build_rename_section(row)

    def build_global_header(self, row):
        ttk.Label(self.main_frame, text="视频批处理工具 V18",
    ...
    def build_rename_section(self, row):
        frame = ttk.LabelFrame(self.main_frame, text="批量重命名（源→目标）", padding=2)
        frame.grid(row=row, column=0, columnspan=3, padx=2, pady=2, sticky="ew")
        frame.columnconfigure(0, weight=1)
        frame.columnconfigure(2, weight=1)

        src_col = ttk.Frame(frame)
        src_col.grid(row=0, column=0, rowspan=2, sticky="nsew", padx=2, pady=2)
        src_col.columnconfigure(1, weight=1)
        src_col.rowconfigure(1, weight=1)

        ttk.Label(src_col, text="源文件夹（点击复制）:").grid(row=0, column=0, sticky="e", padx=2, pady=2)
        ttk.Entry(src_col, textvariable=self.rename_source_folder).grid(row=0, column=1, sticky="ew", padx=2, pady=2)
        ttk.Button(src_col, text="选择", width=6, command=self.select_rename_source).grid(row=0, column=2, padx=2, pady=2)

        src_wrap = ttk.Frame(src_col)
        src_wrap.grid(row=1, column=0, columnspan=3, sticky="nsew", padx=2, pady=2)
        src_wrap.columnconfigure(0, weight=1)
        src_wrap.rowconfigure(0, weight=1)
        self.src_listbox = Listbox(src_wrap, height=6, selectmode=SINGLE, exportselection=False, font=self.ui_font)
        self.src_listbox.grid(row=0, column=0, sticky="nsew")
        self.src_listbox.bind("<ButtonRelease-1>", self.on_rename_src_click)
        sb1 = ttk.Scrollbar(src_wrap, orient="vertical", command=self.src_listbox.yview)
        sb1.grid(row=0, column=1, sticky="ns")
        self.src_listbox.configure(yscrollcommand=sb1.set)

        mid = ttk.Frame(frame)
        mid.grid(row=0, column=1, rowspan=2, sticky="ns", padx=2, pady=2)
        ttk.Label(mid, text="→", font=("Microsoft YaHei", 16)).pack(pady=(8, 4))
        ttk.Label(mid, text="剪贴板:").pack(pady=(0, 2))
        self.clipboard_display = Label(mid, text="(空)", fg="gray", wraplength=120, justify="center", font=self.ui_font)
        self.clipboard_display.pack(pady=(0, 6))
        ttk.Button(mid, text="刷新两列", width=12, command=self.refresh_rename_lists).pack(pady=2)

        dst_col = ttk.Frame(frame)
        dst_col.grid(row=0, column=2, rowspan=2, sticky="nsew", padx=2, pady=2)
        dst_col.columnconfigure(1, weight=1)
        dst_col.rowconfigure(1, weight=1)

        ttk.Label(dst_col, text="目标文件夹（点击粘贴重命名）:").grid(row=0, column=0, sticky="e", padx=2, pady=2)
        ttk.Entry(dst_col, textvariable=self.rename_target_folder).grid(row=0, column=1, sticky="ew", padx=2, pady=2)
        ttk.Button(dst_col, text="选择", width=6, command=self.select_rename_target).grid(row=0, column=2, padx=2, pady=2)

        dst_wrap = ttk.Frame(dst_col)
        dst_wrap.grid(row=1, column=0, columnspan=3, sticky="nsew", padx=2, pady=2)
        dst_wrap.columnconfigure(0, weight=1)
        dst_wrap.rowconfigure(0, weight=1)
        self.dst_listbox = Listbox(dst_wrap, height=6, selectmode=SINGLE, exportselection=False, font=self.ui_font)
        ...
        sb2 = ttk.Scrollbar(dst_wrap, orient="vertical", command=self.dst_listbox.yview)
        sb2.grid(row=0, column=1, sticky="ns")
        self.dst_listbox.configure(yscrollcommand=sb2.set)
    ...
    def apply_video_overlay(self, inp: str, out: str, st: dict):
        ...
        dur = float(st.get("duration_sec", 0)) or probe_duration(FFPROBE_PATH, Path(inp))
        cmd = build_combo_cmd(
            FFMPEG_PATH, FFPROBE_PATH, "video_logo",
            None, Path(inp), Path(logo["path"]), Path(out),
            (0, 0, 0, 0), lpos, dur,
        )
        ok, err = run_ffmpeg(cmd, raise_on_fail=True)
        if not ok:
            raise RuntimeError(err[-300:] if err else "叠加失败")
    ...
    def load_config(self):
        ...
        try:
            ...
            if cfg.get("overlay_state"):
                self._overlay_state = cfg["overlay_state"]
                self._update_overlay_summary()
    ...
    def load_rename_src_list(self):
        ...
        self._src_files = self._list_videos(self.rename_source_folder.get())
    ...
    def load_rename_dst_list(self):
        ...
        files = self._list_videos(self.rename_target_folder.get())
        for f in files:
    ...
    def _rename_dst_single_delayed(self, event):
        self._dst_click_after_id = None
        if not self.clipboard_filename:
            messagebox.showwarning("提示", "请先点击左侧文件复制文件名")
            return
        idx = self._listbox_index_at(self.dst_listbox, event)
        if idx is None:
            return
        self._apply_target_rename(idx, self.clipboard_filename)
    ...
    def on_rename_dst_double_click(self, event):
        ...
        new_name = simpledialog.askstring(
            "确认文件名",
            "确认或修改文件名：",
            initialvalue=self.clipboard_filename,
            parent=self.root,
        )
    ...
    def find_font(self):
        ...
        if platform.system() == "Windows":
            cands = ["C:/Windows/Fonts/msyh.ttc", "C:/Windows/Fonts/arial.ttf", "C:/Windows/Fonts/simhei.ttf"]
        elif platform.system() == "Darwin":
            cands = ["/System/Library/Fonts/PingFang.ttc", "/System/Library/Fonts/Helvetica.ttc"]
        else:
