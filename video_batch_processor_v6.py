...
class VideoBatchProcessor:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(main_frame, text="视频批量处理工具 v6.0", font=("Microsoft YaHei", 16, "bold"))
        ...
        ttk.Label(cut_frame, text="裁切模式:").grid(row=0, column=1, sticky=tk.W, padx=(20,0))
        ...
        # === 结尾落版（带开关）===
        cta_frame = ttk.LabelFrame(main_frame, text="结尾落版", padding="10")
    ...
    def concat_with_cta(self, main_file, cta_file, output_file, cta_has_audio):
        """可靠拼接：先尝试copy，失败则重新编码再concat"""
        try:
            # 尝试1: concat demuxer copy模式（最快）
            ...
            self.log("  copy模式失败，尝试重新编码...")

            # 尝试2: 先统一编码，再concat copy
            w, h, fps = self.get_video_info(main_file)

            # 编码主视频
            temp_main = output_file + ".temp_main.mp4"
            subprocess.run([
            ...
            # concat
            with open(concat_list, "w", encoding="utf-8") as f2:
                f2.write("file '" + temp_main.replace(chr(92), "/") + "'\n")
                f2.write("file '" + temp_cta.replace(chr(92), "/") + "'\n")
    ...
    def process_video(self, input_file, output_file, cut_enable, start_time, end_time, cut_mode,
        ...
        try:
            current_file = input_file  # 当前处理中的文件
            ...
            if wm_enable and wm_text and wm_font and os.path.exists(wm_font):
                ...
                result = subprocess.run(cmd, capture_output=True, text=True)
                if result.returncode != 0:
                    self.log(f"  水印失败: {result.stderr[:100]}")
                    return False, "水印失败"
            ...
            if cta_enable and cta_file and os.path.exists(cta_file):
                self.log("  [CTA] 拼接落版...")
                success = self.concat_with_cta(current_file, cta_file, output_file, cta_has_audio)
                if not success:
                    return False, "CTA拼接失败"
            else:
                self.log("  [CTA] 跳过，直接输出")
                import shutil
