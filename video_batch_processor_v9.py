...
class VideoBatchProcessor:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(main_frame, text="视频批量处理工具 v9.0", font=("Microsoft YaHei", 16, "bold"))
        ...
        ttk.Combobox(wm_frame, textvariable=self.wm_dir_var, 
                     values=["从左往右", "从右往左", "从上往下", "从下往上", 
                             "左上到右下", "右下到左上", "右上到左下", "左下到右上", "波浪", "全场随机"], 
        ...
        ttk.Label(wm_frame, text="字体大小:").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.wm_size_var = tk.IntVar(value=36)
        ttk.Spinbox(wm_frame, from_=12, to=120, increment=2, textvariable=self.wm_size_var, width=8).grid(row=2, column=1, sticky=tk.W, padx=5)
        ttk.Label(wm_frame, text="px").grid(row=2, column=2, sticky=tk.W)

        ttk.Label(wm_frame, text="字体颜色:").grid(row=2, column=3, sticky=tk.W, padx=(20, 0))
        self.wm_color_var = tk.StringVar(value="white")
        ttk.Combobox(wm_frame, textvariable=self.wm_color_var, values=["white", "black", "red", "yellow", "green", "blue"], width=10, state="readonly").grid(row=2, column=4, sticky=tk.W, padx=5)

        # 背景设置
        ttk.Label(wm_frame, text="背景颜色:").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.wm_bg_color_var = tk.StringVar(value="black")
        ttk.Combobox(wm_frame, textvariable=self.wm_bg_color_var, values=["none", "black", "white", "red", "blue", "green"], width=10, state="readonly").grid(row=3, column=1, sticky=tk.W, padx=5)

        ttk.Label(wm_frame, text="背景透明度:").grid(row=3, column=2, sticky=tk.W, padx=(20, 0))
        self.wm_bg_opacity_var = tk.IntVar(value=50)
        ttk.Scale(wm_frame, from_=0, to=100, orient=tk.HORIZONTAL, variable=self.wm_bg_opacity_var, length=120).grid(row=3, column=3, sticky=tk.W, padx=5)
        ttk.Label(wm_frame, textvariable=self.wm_bg_opacity_var).grid(row=3, column=4, sticky=tk.W)
        ttk.Label(wm_frame, text="%").grid(row=3, column=5, sticky=tk.W)

        ttk.Label(wm_frame, text="字体文件:").grid(row=4, column=0, sticky=tk.W, pady=5)
        self.wm_font_var = tk.StringVar(value=self.system_font)
        ttk.Entry(wm_frame, textvariable=self.wm_font_var, width=50).grid(row=4, column=1, columnspan=3, sticky=(tk.W, tk.E), padx=5)
        ttk.Button(wm_frame, text="选择", command=self.select_font).grid(row=4, column=4, padx=5)

        if not self.system_font:
            ttk.Label(wm_frame, text="⚠️ 未检测到系统字体，请手动选择", foreground="red").grid(row=5, column=0, columnspan=5, sticky=tk.W)
        row += 1
    ...
    def process_video(self, input_file, output_file, cut_enable, start_time, end_time, cut_mode,
                     ...
                     wm_enable, wm_text, wm_dir, wm_speed, wm_size, wm_color, wm_border, wm_font, wm_bg_color, wm_bg_opacity):
        try:
            ...
            if wm_enable and wm_text and wm_font and os.path.exists(wm_font):
                ...
                wm_filter = self.build_watermark_filter(wm_text, wm_dir, wm_speed, wm_size, wm_color, wm_border, wm_font, wm_bg_color, wm_bg_opacity)
    ...
    def start_batch_process(self):
        ...
        wm_bg_color = self.wm_bg_color_var.get()
        wm_bg_opacity = self.wm_bg_opacity_var.get()
