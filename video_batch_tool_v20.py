...
class VideoBatchTool:
    ...
    def build_layer_section(self, row, col):
        ...
        ttk.Label(ef, text="截取秒(0=完整):").grid(row=2, column=0, sticky="e", padx=2, pady=2)
        self.ending_trim = StringVar(value="0")
        ttk.Entry(ef, textvariable=self.ending_trim, width=8).grid(row=2, column=1, sticky="w", padx=2, pady=2)
    ...
    def load_selected_template(self) -> None:
        ...
        try:
            ...
            io_mode = "template"
            if tin or tout or cur_in or cur_out:
                ans = messagebox.askyesnocancel(
                    "加载方案模板",
                    f"模板「{name}」与输入/输出文件夹有关，请确认以免处理错素材：\n\n"
                    f"【模板内】\n输入：{tin or '（空）'}\n输出：{tout or '（空）'}\n\n"
                    f"【当前界面】\n输入：{cur_in or '（空）'}\n输出：{cur_out or '（空）'}\n\n"
                    "是 = 沿用模板里的文件夹\n"
                    "否 = 清空输入/输出（换批素材时推荐）\n"
            ...
            io_note = {"template": "沿用模板文件夹", "clear": "已清空输入/输出", "keep": "保持当前文件夹"}[io_mode]
