...
class VideoBatchProcessor:
    def __init__(self, root):
        self.root = root
        self.root.title("静雯的视频批处理工具 V17")
    ...
    def setup_ui(self):
        ...
        ttk.Label(scrollable_frame, text="静雯的视频批处理工具 V17", font=("Microsoft YaHei", 18, "bold")).pack(pady=(0,5))
    ...
    def _to_mpegts(self, inp, out, with_audio):
        """H264 流复制转 TS；其他编码先转 H264 再封装为 TS"""
        is_h264 = self._get_video_codec(inp) == "h264"
        if is_h264:
            if with_audio:
                self.run_ffmpeg(["ffmpeg", "-i", inp, "-c", "copy",
                               "-bsf:v", "h264_mp4toannexb", "-f", "mpegts", "-y", out])
            else:
            ...
            if with_audio:
                self.run_ffmpeg(["ffmpeg", "-i", inp,
                               "-c:v", "libx264", "-preset", "fast", "-crf", "23",
                               "-c:a", "aac", "-f", "mpegts", "-y", out])
            else:
                self.run_ffmpeg(["ffmpeg", "-i", inp, "-f", "lavfi",
                               "-i", "anullsrc=r=48000:cl=stereo", "-shortest",
                               "-c:v", "libx264", "-preset", "fast", "-crf", "23",
    ...
    def add_cta(self, inp, cta, out, keep_audio, trim_sec=0):
        ...
        # 主视频/落版转TS（按编码自动选择流复制或重编码）
        self._to_mpegts(inp, ts1, main_has_audio)
        self._to_mpegts(cta, ts2, cta_has_audio)
    ...
    def process_all(self):
        ...
        if not any([
            self.cut_enable.get(), self.audio_enable.get(),
            self.cta_enable.get(), self.mov_wm_enable.get(), self.txt_wm_enable.get()
        ]):
            messagebox.showwarning("提示", "请至少启用一项处理功能（裁切/音频/落版/MOV水印/文字水印）")
            return

        self.log(f"功能: 裁切={self.cut_enable.get()} 音频={self.audio_enable.get()} "
                 f"落版={self.cta_enable.get()} MOV水印={self.mov_wm_enable.get()} "
                 f"文字水印={self.txt_wm_enable.get()}")
