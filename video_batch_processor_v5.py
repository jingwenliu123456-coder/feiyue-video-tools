...
class VideoBatchProcessor:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(main_frame, text="视频批量处理工具 v5.0", font=("Microsoft YaHei", 16, "bold"))
        ...
        ttk.Label(media_frame, text="(选填)").grid(row=0, column=3, sticky=tk.W, padx=5)
    ...
    def concat_with_cta(self, main_file, cta_file, output_file, cta_has_audio):
        """使用filter_complex concat拼接主视频和CTA，自动处理不同参数"""
        try:
            # 方法1：尝试concat demuxer + copy（最快）
            concat_list = output_file + ".concat.txt"
            with open(concat_list, "w", encoding="utf-8") as f2:
                f2.write(f"file '{main_file.replace(chr(92), '/')}'\n")
                f2.write(f"file '{cta_file.replace(chr(92), '/')}'\n")

            cmd1 = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_list, "-c", "copy", output_file]
            result1 = subprocess.run(cmd1, capture_output=True, text=True)

            if result1.returncode == 0 and os.path.exists(output_file):
                os.remove(concat_list)
                self.log("  CTA拼接成功(copy模式)")
                return True

            self.log("  concat copy失败，尝试filter_complex模式...")
            os.remove(concat_list)

            # 方法2：filter_complex concat（自动处理不同分辨率/帧率）
            if cta_has_audio:
                cmd2 = [
                    "ffmpeg", "-y", "-i", main_file, "-i", cta_file,
            ...
            if result2.returncode == 0 and os.path.exists(output_file):
                self.log("  CTA拼接成功(filter_complex模式)")
                return True
            else:
                self.log(f"  CTA拼接失败: {result2.stderr[:200]}")
                return False

        except Exception as e:
            self.log(f"  CTA拼接异常: {e}")
            return False
    ...
    def process_video(self, input_file, output_file, start_time, end_time, cut_mode, audio_file, cta_file, cta_has_audio, wm_text, wm_dir, wm_speed, wm_y, wm_size, wm_color, wm_border, wm_font):
        try:
            ...
            self.log(f"  裁切模式: {cut_mode}, 段: {start_sec}-{end_sec}s")
            ...
            if cut_mode == "保留":
                ...
                if start_sec > 0:
                    cmd_p1 = ["ffmpeg", "-y", "-i", input_file, "-ss", "0", "-t", str(start_sec), "-c", "copy", temp_part1]
                    subprocess.run(cmd_p1, capture_output=True, text=True)
                else:
                    temp_part1 = None

                if end_sec < total_duration:
                    cmd_p2 = ["ffmpeg", "-y", "-i", input_file, "-ss", str(end_sec), "-c", "copy", temp_part2]
                    subprocess.run(cmd_p2, capture_output=True, text=True)
                else:
                    temp_part2 = None
                ...
                if len(concat_parts) == 0:
                    ...
                    cmd_cc = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", cl, "-c", "copy", temp_merged]
                    subprocess.run(cmd_cc, capture_output=True, text=True)
            ...
            if has_wm:
                ...
                if wm_text and (not wm_font or not os.path.exists(wm_font)):
                    self.log("  警告: 字体未找到，跳过水印")
                temp_after_wm = temp_after_audio

            # === 步骤4: CTA拼接 ===
            if has_cta:
                success = self.concat_with_cta(temp_after_wm, cta_file, output_file, cta_has_audio)
