...
class VideoBatchToolV22(_V21):
    def _ensure_v22_config_vars(self) -> None:
    ...
    def _batch_pipeline_order(self) -> list[str]:  # type: ignore[override]
        """优先裂变自定义顺序；否则按 ⊞ 布局：上→下、左→右（跳过预览格）。"""
    ...
    def _build_v22_module(self, key: str, row: int, col: int, *, rowspan: int = 1) -> None:
        builders = {
            ...
            "enhance": self.build_enhance_section,
...
def main():
    ...
    try:
        ...
        messagebox.showerror(
            ...
            f"程序启动失败：\n{e}\n\n"
            "若刚更新过版本，可删除配置后重试：\n"
            "~/Library/Application Support/HabiVideoTool/video_batch_config_v21.json",
            parent=root,
        )
        raise
