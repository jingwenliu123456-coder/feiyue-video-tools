...
class VideoBatchProcessor:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(main_frame, text="视频批量处理工具 v3.0", font=("Microsoft YaHei", 16, "bold"))
        ...
        ttk.Label(cut_frame, text="开始时间:").grid(row=0, column=0, sticky=tk.W)
        self.start_time_var = tk.StringVar(value="00:00")
        ttk.Entry(cut_frame, textvariable=self.start_time_var, width=10).grid(row=0, column=1, sticky=tk.W, padx=5)

        ttk.Label(cut_frame, text="结束时间:").grid(row=0, column=2, sticky=tk.W, padx=(20, 0))
        self.end_time_var = tk.StringVar(value="00:15")
        ttk.Entry(cut_frame, textvariable=self.end_time_var, width=10).grid(row=0, column=3, sticky=tk.W, padx=5)

        ttk.Label(cut_frame, text="(格式: 秒 或 分:秒)").grid(row=0, column=4, sticky=tk.W, padx=10)
        row += 1
    ...
    def process_video(self, input_file, output_file, start_time, end_time, audio_file, cta_file, cta_has_audio, wm_text, wm_dir, wm_speed, wm_y, wm_size, wm_color, wm_border, wm_font):
        try:
            ...
            cmd_audio = ["ffmpeg", "-y", "-i", temp_cut, "-i", audio_file, "-c:v", "copy", "-c:a", "aac", "-map", "0:v:0", "-map", "1:a:0", "-shortest", temp_audio]
            ...
            if has_wm:
                ...
                self.log(f"水印滤镜: {wm_filter[:80]}...")
                cmd_wm = ["ffmpeg", "-y", "-i", temp_audio, "-vf", wm_filter, "-c:a", "copy", temp_watermark]
                result_wm = subprocess.run(cmd_wm, capture_output=True, text=True)
                if result_wm.returncode != 0:
                    self.log(f"水印添加失败: {result_wm.stderr[:200]}")
                    return False, "水印添加失败"
                source_for_concat = temp_watermark
            else:
                if wm_text and (not wm_font or not os.path.exists(wm_font)):
                    self.log("警告: 未找到字体文件，跳过水印")
                source_for_concat = temp_audio

            concat_list = output_file + ".concat.txt"
            with open(concat_list, "w", encoding="utf-8") as f2:
                f2.write(f"file '{source_for_concat.replace(chr(92), '/')}'\n")
                f2.write(f"file '{cta_file.replace(chr(92), '/')}'\n")

            if not cta_has_audio:
                temp_cta = output_file + ".temp_cta.mp4"
                cmd_cta_mute = ["ffmpeg", "-y", "-i", cta_file, "-c:v", "copy", "-an", temp_cta]
                subprocess.run(cmd_cta_mute, capture_output=True, text=True)
                with open(concat_list, "w", encoding="utf-8") as f2:
                    f2.write(f"file '{source_for_concat.replace(chr(92), '/')}'\n")
                    f2.write(f"file '{temp_cta.replace(chr(92), '/')}'\n")

            cmd_concat = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_list, "-c", "copy", output_file]
            result = subprocess.run(cmd_concat, capture_output=True, text=True)

            for temp_file in [temp_cut, temp_audio, concat_list]:
                if os.path.exists(temp_file):
                    os.remove(temp_file)
            if has_wm and os.path.exists(temp_watermark):
                os.remove(temp_watermark)
            if not cta_has_audio:
                temp_cta = output_file + ".temp_cta.mp4"
                if os.path.exists(temp_cta):
                    os.remove(temp_cta)
    ...
    def start_batch_process(self):
        ...
        if not all([input_folder, output_folder, audio_file, cta_file]):
        ...
        self.log(f"裁切: {start_time} -> {end_time}")
        self.log(f"音频: {audio_file}")
        self.log(f"CTA: {cta_file}")
        if wm_text:
            self.log(f"动态水印: {wm_text} ({wm_dir}, {wm_speed}px/s)")
        self.log("=" * 50)

        thread = threading.Thread(target=self.process_all, args=(input_folder, output_folder, start_time, end_time, audio_file, cta_file, cta_has_audio, wm_text, wm_dir, wm_speed, wm_y, wm_size, wm_color, wm_border, wm_font))
        thread.daemon = True
        thread.start()

    def process_all(self, input_folder, output_folder, start_time, end_time, audio_file, cta_file, cta_has_audio, wm_text, wm_dir, wm_speed, wm_y, wm_size, wm_color, wm_border, wm_font):
        video_extensions = [".mp4", ".mov", ".avi", ".mkv", ".m4v"]
        ...
        for i, filename in enumerate(video_files, 1):
            ...
            for item in self.tree.get_children():
                if self.tree.item(item, "values")[0] == filename:
                    ...
                    break
            success, msg = self.process_video(input_path, output_path, start_time, end_time, audio_file, cta_file, cta_has_audio, wm_text, wm_dir, wm_speed, wm_y, wm_size, wm_color, wm_border, wm_font)
