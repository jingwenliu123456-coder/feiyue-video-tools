...
"""
视频批处理工具 V19
跨平台 Windows/macOS | 自由画布叠加 | 双模式重命名 | 自定义输出命名
"""
...
from ui.naming_convention_tab import NamingConventionPanel
...
from core.overlay_engine import build_combo_cmd, probe_duration, resolve_duration, format_ffmpeg_stderr, list_videos_in_folder
from modules.platform_utils import (
    SYSTEM, resolve_ffmpeg, check_ffmpeg_available, path_for_ffmpeg,
    find_default_font, open_folder, config_path,
)
from modules.output_naming import (
    generate_output_filename, preview_names, unique_path, append_rename_file,
)

FFMPEG_PATH, FFPROBE_PATH = resolve_ffmpeg()
CONFIG_FILE = str(config_path("video_batch_config_v19.json"))
...
class VideoBatchTool:
    def __init__(self, root):
        ...
        self._naming_convention_state: dict = {}
        self._paned_sash_pos = 0

        # === 严格分离：全局 I/O vs 重命名模块 ===
        self.global_input_folder = StringVar()
        self.global_output_folder = StringVar()
        self.rename_source_folder = StringVar()
        self.rename_target_folder = StringVar()
    ...
    def create_scrollable_canvas(self):
        ...
        self.paned = ttk.Panedwindow(self.outer_frame, orient=VERTICAL)
        self.paned.pack(fill=BOTH, expand=True)

        self.upper_pane = ttk.Frame(self.paned)
        self.lower_pane = ttk.Frame(self.paned)
        self.paned.add(self.upper_pane, weight=3)
        self.paned.add(self.lower_pane, weight=1)
        try:
            self.paned.paneconfigure(self.upper_pane, minsize=UPPER_PANE_MIN_HEIGHT)
            self.paned.paneconfigure(self.lower_pane, minsize=RENAME_PANE_MIN_HEIGHT)
        except Exception:
            pass

        self.canvas = Canvas(self.upper_pane, highlightthickness=0)
        self.canvas.pack(side=LEFT, fill=BOTH, expand=True)
        scrollbar = ttk.Scrollbar(self.upper_pane, orient="vertical", command=self.canvas.yview)
        ...
        def _on_mousewheel(event):
            if self._mouse_over_rename_pane(event):
                return
            self.canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

        self.root.bind_all("<MouseWheel>", _on_mousewheel)

        self.rename_pane_frame = ttk.Frame(self.lower_pane)
        self.rename_pane_frame.pack(fill=BOTH, expand=True)

        self._create_sash_handle()
        self.paned.bind("<Configure>", self._position_sash_handle)
        self.paned.bind("<B1-Motion>", self._position_sash_handle)
        self.paned.bind("<ButtonRelease-1>", self._on_paned_sash_release)
        self.root.bind("<Configure>", self._position_sash_handle)
        self.root.after(200, self._restore_paned_sash)
    ...
    def build_ui(self):
        ...
        self.build_rename_section()

    def build_global_header(self, row):
        ttk.Label(self.main_frame, text="视频批处理工具 V19",
    ...
    def build_global_io(self, row):
        ...
        nf_outer = ttk.LabelFrame(frame, text="输出与命名", padding=4)
        nf_outer.grid(row=2, column=0, columnspan=4, sticky="ew", pady=(6, 2))
        nf_outer.columnconfigure(0, weight=1)
        nf_outer.rowconfigure(0, weight=1)

        self.naming_notebook = ttk.Notebook(nf_outer)
        self.naming_notebook.grid(row=0, column=0, sticky="nsew")

        tab_batch = ttk.Frame(self.naming_notebook, padding=4)
        self.naming_notebook.add(tab_batch, text="批量输出命名")
        nf = tab_batch
        nf.columnconfigure(1, weight=1)

        tab_std = ttk.Frame(self.naming_notebook, padding=4)
        self.naming_notebook.add(tab_std, text="命名规范")
        tab_std.columnconfigure(0, weight=1)
        tab_std.rowconfigure(0, weight=1)
        try:
            self.naming_convention_panel = NamingConventionPanel(
                tab_std,
                log_fn=self.log,
                on_change=lambda st: setattr(self, "_naming_convention_state", st),
            )
            self.naming_convention_panel.grid(row=0, column=0, sticky="nsew")
        except Exception as e:
            self._log_exception("naming_convention_panel_init", e)
            ttk.Label(
                tab_std,
                text=f"命名规范面板加载失败：{e}\n详见 {ERROR_LOG_FILE}",
                foreground="red",
            ).grid(row=0, column=0, sticky="nw", padx=8, pady=8)
            self.naming_convention_panel = None

        self.custom_naming_enable = BooleanVar(value=False)
        self.output_prefix_mode = StringVar(value="none")
        self.output_prefix_custom = StringVar(value="")
        ttk.Checkbutton(nf, text="启用自定义命名格式", variable=self.custom_naming_enable,
                        command=self._update_naming_preview).grid(row=0, column=0, columnspan=4, sticky="w")
        ttk.Label(nf, text="格式:").grid(row=1, column=0, sticky="e", padx=2)
        self.naming_format = StringVar(value="{name}")
        ne = ttk.Entry(nf, textvariable=self.naming_format, width=40)
        ne.grid(row=1, column=1, sticky="ew", padx=2)
        ne.bind("<KeyRelease>", lambda _e: self._update_naming_preview())
        ttk.Label(nf, text="起始编号:").grid(row=1, column=2, sticky="e")
        self.naming_start_index = StringVar(value="1")
        ttk.Entry(nf, textvariable=self.naming_start_index, width=5).grid(row=1, column=3, padx=2)
        ttk.Label(nf, text="变量: {name} {index} {index:03d} {date} {time} | 启用后按格式输出，忽略前缀",
                  font=("", 7), foreground="gray").grid(row=2, column=0, columnspan=4, sticky="w")
        self.naming_preview = StringVar(value="预览：—")
        ttk.Label(nf, textvariable=self.naming_preview, font=("", 8), foreground="#2e7d32").grid(
            row=3, column=0, columnspan=4, sticky="w", pady=2)
        pf = ttk.Frame(nf)
        pf.grid(row=4, column=0, columnspan=4, sticky="w", pady=(4, 2))
        ttk.Label(pf, text="输出前缀:").pack(side=LEFT, padx=(0, 4))
        ttk.Radiobutton(pf, text="无（保持原名）", variable=self.output_prefix_mode, value="none",
                        command=self._update_naming_preview).pack(side=LEFT, padx=2)
        ttk.Radiobutton(pf, text="使用底图名", variable=self.output_prefix_mode, value="bg_name",
                        command=self._update_naming_preview).pack(side=LEFT, padx=2)
        ttk.Radiobutton(pf, text="自定义", variable=self.output_prefix_mode, value="custom",
                        command=self._update_naming_preview).pack(side=LEFT, padx=2)
        pe = ttk.Entry(pf, textvariable=self.output_prefix_custom, width=14)
        pe.pack(side=LEFT, padx=4)
        pe.bind("<KeyRelease>", lambda _e: self._update_naming_preview())
        return row + 1
    ...
    def save_config(self):
        cfg = {k: v.get() for k, v in [
            ...
            ("rename_source", self.rename_source_folder),
            ("rename_target", self.rename_target_folder),
            ...
            ("append_submode", self.append_submode),
            ("append_position", self.append_position),
            ("append_string", self.append_string),
            ("custom_naming_enable", self.custom_naming_enable),
            ("naming_format", self.naming_format),
            ("naming_start_index", self.naming_start_index),
            ("output_prefix_mode", self.output_prefix_mode),
            ("output_prefix_custom", self.output_prefix_custom),
        ]}
        if self._overlay_state:
            cfg["overlay_state"] = self._overlay_state
        if getattr(self, "naming_convention_panel", None):
            cfg["naming_convention"] = self.naming_convention_panel.get_state()
        self._save_paned_sash()
        if self._paned_sash_pos:
            cfg["paned_sash_pos"] = self._paned_sash_pos
        try:
            with open(CONFIG_FILE, "w", encoding="utf-8") as f:
                json.dump(cfg, f, ensure_ascii=False, indent=2)
    ...
    def load_config(self):
        ...
        try:
            mapping = [
                ...
                (self.rename_source_folder, "rename_source"),
                (self.rename_target_folder, "rename_target"),
                ...
                (self.rename_mode, "rename_mode", "replace"),
                (self.append_submode, "append_submode", "auto"),
                (self.append_position, "append_position", "end"),
                (self.append_string, "append_string", ""),
                (self.custom_naming_enable, "custom_naming_enable", False),
                (self.naming_format, "naming_format", "{name}"),
                (self.naming_start_index, "naming_start_index", "1"),
                (self.output_prefix_mode, "output_prefix_mode", "none"),
                (self.output_prefix_custom, "output_prefix_custom", ""),
            ]
            for item in mapping:
            ...
            if isinstance(overlay_st, dict):
                ...
                self._overlay_state = {}
            if cfg.get("naming_convention") and getattr(self, "naming_convention_panel", None):
                try:
                    self.naming_convention_panel.load_state(cfg["naming_convention"])
                except Exception as e:
                    self._log_exception("load_naming_convention", e)
            if cfg.get("paned_sash_pos"):
                try:
                    self._paned_sash_pos = int(cfg["paned_sash_pos"])
                except (TypeError, ValueError):
                    pass
            self.root.after_idle(self._restore_paned_sash)
            if self.rename_source_folder.get():
                self.load_rename_src_list()
    ...
    def _naming_start_idx(self) -> int:
        try:
            return int(self.naming_start_index.get() or 1)
        except ValueError:
            return 1

    def get_output_name_prefix(self, bg_path: Path | None = None) -> str:
        """未启用自定义命名时，为输出文件名添加可选前缀"""
        if self.custom_naming_enable.get():
            return ""
        mode = self.output_prefix_mode.get()
        if mode == "none":
            return ""
        if mode == "bg_name":
            if bg_path:
                return f"{bg_path.stem}_"
            layers = (self._overlay_state or {}).get("layers", {})
            bp = layers.get("bg", {}).get("path", "")
            if bp:
                return f"{Path(bp).stem}_"
            return ""
        if mode == "custom":
            p = (self.output_prefix_custom.get() or "").strip()
    ...
    def make_batch_output_name(self, original_name: str, index: int, prefix: str = "") -> str:
        return generate_output_filename(
            original_name, index,
            self.naming_format.get(),
            self.custom_naming_enable.get(),
            prefix,
        )
    ...
    def _update_naming_preview(self):
        in_dir = self.global_input_folder.get()
        files = self._list_videos(in_dir) if in_dir and os.path.isdir(in_dir) else ["sample_01.mp4", "sample_02.mp4"]
        start = self._naming_start_idx()
        prefix = self.get_output_name_prefix()
        lines = preview_names(
            files, self.naming_format.get(), start,
            self.custom_naming_enable.get(), prefix, limit=3,
        )
        self.naming_preview.set("预览：" + " | ".join(lines) if lines else "预览：—")

    def _apply_batch_naming_to_module(self, mod):
        mod.set_batch_naming(
            self._overlay_name_resolver,
            self._naming_start_idx(),
            prefix_fn=lambda combo: self.get_output_name_prefix(mod.bg_path),
        )
            ...
            for idx, name in enumerate(files, 1):
                ...
                try:
                    ...
                    if self.overlay_enable.get():
                        ...
                        layers = st.get("layers", {})
                        if layers.get("bg", {}).get("enabled"):
                            raise RuntimeError("含底图的叠加请使用「叠加批量处理」")
                        if not layers.get("logo", {}).get("enabled"):
                            raise RuntimeError("一键批处理叠加需要启用 Logo 层")
                        tmp = self.get_temp(out, "overlay")
                        self.apply_video_overlay(current, tmp, st)
                        if current != inp:
                            temps.append(current)
                        current = tmp
                        pos = layers["logo"]["position"]
                        self.log(f"  视频+Logo叠加完成 ({pos['x']},{pos['y']} {pos['w']}×{pos['h']})")
                    ...
                    if self.ending_enable.get():
                        ...
                        trim_sec = int(self.ending_trim.get() or "0")
    ...
    def replace_audio(self, inp, audio, out):
        self.ffmpeg([FFMPEG_PATH, "-i", inp, "-i", audio,
