...
# ---------- 1. 替换文字水印UI区域：从param_frame中移除，改为独立区域 ----------
...
new_txt_ui = '''        # === 动态文字水印（独立区域，v9丰富效果移植）===
        ...
        if not self.txt_wm_font.get():
            ttk.Label(txt_wm_frame, text="⚠️ 未检测到系统字体，请手动选择", foreground="red").grid(row=6, column=0, columnspan=5, sticky=tk.W)
'''

content = content.replace(old_txt_ui, new_txt_ui)

# ---------- 2. 在 select_mov_wm 后添加 select_font ----------
...
with open(r"D:\其他软件\我的\_diff_v17_watermark.patch", "w", encoding="utf-8") as out:
    out.write("".join(diff))
