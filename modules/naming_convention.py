    ...
    *,
    ...
    if n > 0 and result:
        ...
        if len(parts) > n:
            ...
            warnings.append(f"「-」不足 {n} 个，无法截取第{n}个之后，已保留原名")
...
def merge_legacy_with_fields(
    ...
    *,
    ...
    use_legacy_first = legacy_priority or bool(ov)
    ...
    use_stem = _should_preprocess_legacy_stem(
        ...
        dash_keep_after=dash_keep_after,
    )
    ...
    if use_stem:
        ...
        if cleaned_stem and not parsed.parse_ok:
            tag_parts = [p.strip() for p in cleaned_stem.split("-") if p.strip()][:3]
            if not tag_parts:
                tag_parts = [cleaned_stem.strip()] if cleaned_stem.strip() else []
            parsed = _clone_parsed_with_tags(parsed, tag_parts, tag_library=tag_library)
        resolve_strip = None
        # 第 N 个「-」之后截取到的段落，即使不在标准库也允许写进新名
        if int(dash_keep_after or 0) > 0 and parsed.tags:
