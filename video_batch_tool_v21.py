...
class VideoBatchToolV21(v20.VideoBatchTool):
    def _ensure_core_config_vars(self) -> None:
        """V22 等可隐藏模块布局，但配置/批处理/预览仍依赖这些变量，启动前必须存在。"""
        def _bool(name: str, default: bool = False) -> None:
            if not hasattr(self, name):
                setattr(self, name, BooleanVar(value=default))

        def _str(name: str, default: str = "") -> None:
            if not hasattr(self, name):
                setattr(self, name, StringVar(value=default))

        _bool("cut_enable")
        _str("cut_mode", "保留")
        _str("cut_start", "00:00")
        _str("cut_end", "00:15")
        _str("cut_range_mode", "固定时段")
        _str("cut_tail_sec", "5")

        _bool("ratio_enable")
        _str("ratio_target", "9:16")
        ...
        _str("mov_watermark_path")
        _str("mov_watermark_mode", "fullscreen")
        _str("mov_watermark_x", "0")
        _str("mov_watermark_y", "0")
        _str("mov_watermark_w", "0")
        _str("mov_watermark_h", "0")
        _str("mov_watermark_duration", "0")
        _str("mov_res_info", "分辨率: 未检测")

        _bool("audio_enable")
        _str("audio_path_var")
        _bool("png_wm_enable")
        _str("png_wm_path")
        _str("png_wm_mode", "fullscreen")
        _str("png_wm_position", "居中")
        _str("png_wm_x", "0")
        _str("png_wm_y", "0")
        _str("png_wm_w", "0")
        _str("png_wm_h", "0")
        _str("png_wm_time_mode", "全程")
        _str("png_wm_time_start", "0")
        _str("png_wm_time_end", "5")

        _bool("layer_enable")
        _str("layer_type", "浮层落版")
        _bool("logo_enable")
        _str("logo_path_var")
        _str("logo_mode", "结尾覆盖落版")
        _str("logo_ratio", "9:16")
        _str("logo_position", "居中")
        _str("logo_size_mode", "百分比")
        _str("logo_size_value", "100")
        _str("overlay_custom_x", "0")
        _str("overlay_custom_y", "0")
        _str("ending_trim", "1.0")
        _str("endcard_info", "主视频: 未检测 | 落版: 未检测")
        _bool("overlay_keep_audio")
        _str("overlay_time_mode", "全程")
        _str("overlay_time_start", "0")
        _str("overlay_time_end", "5")

        _bool("ending_enable")
        _bool("ending_keep_audio")

        _bool("overlay_enable")
        _str("overlay_summary", "未配置 — 点击打开编辑器")

        self._init_v20_logo_compat_vars()

    def _is_enabled(self, attr: str) -> bool:
    ...
    def _batch_step_enabled(self, key: str) -> bool:
        ...
        return self._is_enabled(attr) if attr else False
    ...
    def _on_overlay_mode_change(self):
        if hasattr(self, "logo_mode"):
            self.logo_mode.set("结尾覆盖落版")
        if hasattr(self, "logo_size_value"):
            self.logo_size_value.set("100")
        panel = getattr(self, "_panel_endcard", None)
        if panel is not None:
            panel.grid()
        if hasattr(self, "_refresh_endcard_timeline"):
    ...
    def save_config(self):
        ...
        try:
            ...
            cfg = {}
        cfg.update(self._v21_extra_into_dict())
        cfg["log_pane_height"] = getattr(self, "_log_pane_height", LOG_PANE_DEFAULT)
        cfg["ui_theme"] = getattr(self.root, "_ui_theme", getattr(self.root, "_bootstrap_theme", "darkly"))
        try:
            with open(v20.CONFIG_FILE, "w", encoding="utf-8") as f:
                json.dump(cfg, f, ensure_ascii=False, indent=2)
        except OSError:
    ...
    def _apply_v21_extra_fields(self, cfg: dict) -> None:
        ...
        try:
            ...
            pass
        try:
            from modules.fission_engine import pipeline_steps_from_fission_order
            steps = pipeline_steps_from_fission_order(
                cfg.get("_fission_func_order"),
                default_steps=self._BATCH_PIPELINE_DEFAULT,
            )
            self._pipeline_order_override = steps
        except Exception:
    ...
    def _apply_config_dict(self, cfg: dict, *, io_mode: str = "template") -> None:  # type: ignore[override]
        ...
        self._apply_v21_extra_fields(cfg if isinstance(cfg, dict) else {})
    ...
    def load_config(self):
        ...
        try:
            ...
            if hasattr(self, "logo_mode"):
                self.logo_mode.set("结尾覆盖落版")
            self._apply_v21_extra_fields(cfg)
            self._pipeline_order_override = None
    ...
    def apply_overlay_endcard(self, inp: str, overlay_path: str, out: str) -> None:
        ...
        rot_note = ""
        if main_rot or ov_rot:
            rot_note = f" | 旋转归一 主:{main_rot}° 落版:{ov_rot}°"
        if ov_has and not main_has:
            audio_note = " | 落版音频: 主片无声→强制保留落版音"
        elif ov_has and keep_audio:
            audio_note = " | 落版音频: 主片有声→重叠段 amix 混合"
        elif ov_has and extend > 0:
            audio_note = " | 落版音频: 延长段自动保留"
        elif ov_has:
            audio_note = " | 落版有音轨（未勾选「保留落版音频」则重叠段不混入）"
        else:
            audio_note = " | 落版无音轨"
        self.log(
            f"  浮层落版时长: 主视频 {main_dur:.2f}s | 落版 {logo_dur:.2f}s | "
            f"第 {start_time:.2f}s 起叠 | 延长 {extend:.2f}s | 输出 {total_dur:.2f}s"
            f" | 画幅 {vw}x{vh}{rot_note}{audio_note}"
        )
        # 视频 setpts + 音频 adelay 对齐；禁止 -itsoffset（会与 amix 冲突，主片有声时落版音常丢）
        ...
        cmd = [
            v20.FFMPEG_PATH, "-y",
            "-i", inp,
            *self._overlay_input_args(overlay_path, itsoffset=0.0, duration=ov_input_dur),
            "-filter_complex", filt,
            "-map", "[v]",
            "-map", audio_map,
            *v20.VENC,
        ]
        # 延长时禁止 -c:a copy（会按短音轨截断画面/无声）
        if audio_map == "0:a?" and extend <= 0:
            cmd.extend(["-c:a", "copy"])
        else:
            cmd.extend(v20.AENC)
        # 清掉旋转标签，避免播放器二次旋转
        cmd.extend(["-metadata:s:v:0", "rotate=0", "-t", f"{total_dur}", out])
        self.ffmpeg(cmd)
    ...
    def process_batch(self, *, silent: bool = False):
        ...
        try:
            ...
            ov_mode = (
                (getattr(self, "logo_mode", None) or StringVar(value="")).get()
                if self._is_enabled("logo_enable") else ""
            )
            ...
            self.log(
                f"启用: 裁切={self._is_enabled('cut_enable')} "
                f"比例={self._is_enabled('ratio_enable')} "
                f"浮层落版={self._is_enabled('logo_enable')}({ov_mode}) "
                f"MOV水印={self._is_enabled('enable_mov_watermark')} "
                f"PNG水印={self._is_enabled('png_wm_enable')} "
                f"画布叠加={self._is_enabled('overlay_enable')} "
